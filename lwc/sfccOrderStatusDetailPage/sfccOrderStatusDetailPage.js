...
export default class Scc_orderStatusLWC extends LightningElement {
      ...
      setCurrentPageRef(pageRef) {
        ...
        if (pageRef.state.Source) {
         ...
         if (pageRef.state.defaultFieldValues) {      
            ...
            let Search = decodedValues.Search;
            this.SearchByvalue = Search;
            let even = { target: { value: Search } };
            this.handleSearchOptionChange(even);
            this.PONum = decodedValues.PONum;
            this.ISBnNum = decodedValues.ISBnNum;
            this.startDate = decodedValues.startDate
            this.endDate = decodedValues.endDate;
            this.OrderStatusValue = decodedValues.OrderStatusValue;
            let ordStatusEve = { detail: { value: decodedValues.OrderStatusValue } };
            this.handleOrderStatusOptionChange(ordStatusEve);
            this.InvoNum = decodedValues.InvoNum;
            this.ZipNum = decodedValues.ZipNum;
            this.StateNum = decodedValues .StateNum;
            this.CountryValue = decodedValues.CountryValue;
            let countStatusEve = { target: { value: decodedValues.CountryValue } };
            this.handleCountryOptionChange(countStatusEve);
            console.log('Country', this.CountryValue);
            this.DocContrNum =  decodedValues.DocContrNum; 
            this.handleSearch();  
 ...
 //Variables initialization
   ...
   month: "numeric",
    ...
    connectedCallback() {
        ...
        if (this.getdatafrmhm) {
            ...
            let ordStatusEve = { detail: { value: this.getdatafrmhm.OrderStatusValue } };
            this.handleOrderStatusOptionChange(ordStatusEve);
            ...
            let countStatusEve = { target: { value: this.getdatafrmhm.CountryValue } };
            this.handleCountryOptionChange(countStatusEve);
            this.DocContrNum =  this.getdatafrmhm.DocContrNum;//added by vaibhav for bug fix
            this.handleSearch();
        }
  ...
  isRenderedCallbackCalled = false
    ...
    if (this.source == 'comp' && this.isRenderedCallbackCalled==false){    
               console.log('Source');
                ...
                for (let i = 0; i < data.ITEMS.asList.length; i++) {
                    ...
                    if(this.statusData[data.ITEM_STATUS.asList[i].ItemSTAT].toUpperCase() == 'SHIPPED') {
                        ...
                        this.shipItemstoCheck.push({
                            [i]:data.ITEM_STATUS.asList[i].ISBN
                        });
                    }else{
                       this.QtyClosed = '';
    ...
    updateOrderLineStatus(){
        ...
        for(let i=0;i<this.orderLineRecords.length;i++){
            if(this.orderLineRecords[i].LineStatus.toUpperCase() =='SHIPPED'){
                if(this.shipmentNumbers.length < 0){
                    this.orderLineRecords[i].LineStatus = 'Invoiced';
                }
            }
        }
    }
