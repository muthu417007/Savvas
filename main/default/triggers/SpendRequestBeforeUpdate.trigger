trigger SpendRequestBeforeUpdate on Gifting_Request__c (before Update) {
  ...
  for( Gifting_Request__c gr : Trigger.new )
    {
     Gifting_Request__c oldgr = Trigger.oldMap.get(gr.id);
     
     //update the approvers if they have been modified
      if( gr.Department__c != oldgr.Department__c || gr.Sales_Market__c != oldgr.Sales_Market__c)
      {
  ...
  //System.debug('Amount_per_person__c =' + gr.Amount_per_person__c + ' Total_Expected_Amount__c =' + gr.Total_Expected_Amount__c + ' Submitted_Timestamp__c=' + gr.Submitted_Timestamp__c + ' Automatic_MGM_Approval__c=' + gr.Automatic_MGM_Approval__c + ' MGM_Approval__c=' + gr.MGM_Approval__c);
